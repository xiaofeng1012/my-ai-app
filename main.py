...
# --- 1. 基礎設定 ---
st.set_page_config(page_title="KSR Platform", layout="wide", page_icon="📡")
apply_ksr_styles()
...
sel_lang = st.sidebar.selectbox("Select", ["繁體中文", "English"], label_visibility="collapsed")
...
st.sidebar.caption("Version 8.1.0-MODULAR")
...
global_devices[display_id] = {
    "name": display_id, "city": loc['city'] if loc else "Unknown", 
    "lat": loc['lat'] if loc else 25.03, "lon": loc['lon'] if loc else 121.56,
    "last": datetime.now().strftime("%H:%M:%S"), "ts": time.time()
}
...
with c_diag:
    ...
    csv_data = generate_csv_report(df_raw, sys_hash, display_id)
    st.download_button(f"📥 {L['export_btn']}", csv_data, f"KSR_Audit_{display_id}.csv", "text/csv")

with c_map:
    st.subheader(f"🗺️ {L['map_title']}")
    map_df = pd.DataFrame([{"lat": v['lat'], "lon": v['lon'], "name": v['name']} for v in global_devices.values()])
...
st.markdown(f'<div class="ksr-footer">DEVELOPED BY KSR R&D TEAM &copy; 2026.</div>', unsafe_allow_html=True)
