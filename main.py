...
def get_audit_csv(df, info_dict):
    ...
    output.write(f"報告編號: {info_dict['hash']}\n")
    output.write(f"採樣時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    output.write(f"平均延遲: {df['ms'].mean():.2f} ms | Jitter: {info_dict['jitter']:.2f} ms\n")
    output.write(f"--------------------------\n\n")
...
st.sidebar.caption("Version 6.8.2-PRO | 自製監測系統")
...
st.markdown(f"**系統稽核編號:** `{sys_hash}` | **監測節點:** `{loc['city'] if loc else '自動定位'}`")
...
with c_diag:
    ...
    fig.update_layout(height=350, margin=dict(l=0, r=0, t=10, b=0), xaxis_title="時間", yaxis_title="延遲 (ms)")
...
list_data = [{"單位名稱": v['display_name'], "地理位置": v['city'], "最後在線紀錄": v['last_seen']} for v in global_devices.values()]
